...
parser.add_argument('--ena-dwt-ratio', action='store_true', default=False,
                    help='Learnable Deep Format')
parser.add_argument('--post-norm', action='store_true', default=False,
                    help='Learnable Deep Format')
...
def validate(args):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                if args.aux_header == True:
                    output = model(input, ena_dwt_ratio=args.ena_dwt_ratio, dwt_quant=args.dwt_quant)[0]
                else:
                    if args.vit == False:
    ...
    results = OrderedDict(
        ...
        crop_pct=crop_pct,
        interpolation=data_config['interpolation'],
        sfn=args.sfn + ' / ' + args.corrupted + ' / ' + args.img_mode, 
    )
