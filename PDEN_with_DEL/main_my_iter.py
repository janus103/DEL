        ...
        for epoch in range(tgt_epochs):
            ...
            for i, (x, y) in enumerate(trloader):
                ...
                if flag_fixG:
                    ...
                    loss_w = w_cls * tgt_cls_loss.clone()
                    loss_d = w_div * div_loss.clone() # minus
                    loss_cyc = w_cyc * cyc_loss.clone() 
                    loss_info = w_info * con_loss_adv.clone()
                    loss = loss_w.clone() - loss_d.clone() + loss_cyc.clone() + loss_info.clone()
                    #print(f'{loss_w} / {loss_w.shape}')
                    #print(f'{loss_d} / {loss_d.shape}')
                    #print(f'{loss_cyc} / {loss_cyc.shape}')
                    #print(f'{loss_info} / {loss_info.shape}')
                    g1_opt.zero_grad()
