...
def make_figure_out(tensor_dwt_lst):
    ...
    tensor_dwt_lst[0] = (torch.round(tensor_dwt_lst[0] / 2).to(torch.int32)/255)
    tensor_dwt_lst[1] = (torch.round(tensor_dwt_lst[1] + 127.5).to(torch.int32)/255)
    tensor_dwt_lst[2] = (torch.round(tensor_dwt_lst[2] + 127.5).to(torch.int32)/255)
...
def get_dwt_level2(x: torch.Tensor, lst, x_dwt_rate = None, x_bias: torch.Tensor = None):
...
def get_dwt_level2_inverse(lst):
    ifm = DWTInverse(mode='zero', wave='db1').cuda()
    B, C, W, H = lst[0].shape
