...
group.add_argument('--post-norm', action='store_true', default=False,
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            if args.aux_header == True:
                output = model(input)
                loss, loss_dwt = loss_fn(output[0], target, output[1], target_dwt)
                loss_dwt=loss_dwt.type(torch.float32)
                loss_dwt=loss_dwt.cuda()
                loss=loss.type(torch.float32)
                loss=loss.cuda()
                loss = loss + loss_dwt
            elif args.no_skip == True:
                output, loss_dwt = model(input, dwt_ratio=target_dwt, ena_dwt_ratio=args.ena_dwt_ratio, post_norm=args.post_norm, dwt_quant=args.dwt_quant, dwt_drop=args.drop_low)
                loss = loss_fn(output, target) + loss_dwt
            else:
                if args.mvar == True:
                    output, gt_mean_var = model(input, dwt_ratio=target_dwt, ena_dwt_ratio=args.ena_dwt_ratio, post_norm=args.post_norm, dwt_quant=args.dwt_quant, dwt_drop=args.drop_low)
                    output[1] = torch.stack(output[1],dim=0)        
                    combine_tensor = torch.stack([torch.stack(lst, dim=1) for lst in gt_mean_var], dim=0).detach() # shape = 16, B, 2 
                    #loss2 = msle_loss(output[1], combine_tensor, device)
                    loss2 = 0
                    loss = loss_fn(output[0], target) +  loss2
                elif args.vit == True:
                    output = model(input)
                    loss = loss_fn(output, target)
                else:
                    output = model(input, dwt_ratio=target_dwt, ena_dwt_ratio=args.ena_dwt_ratio, post_norm=args.post_norm, dwt_quant=args.dwt_quant, dwt_drop=args.drop_low)
                    loss = loss_fn(output, target)
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                if args.vit == True:
                    ...
                    output = model(input, post_norm=args.post_norm, dwt_quant=args.dwt_quant)
                if args.mvar == True:
                    gt_mvar = output[1]
                    output = output[0]
