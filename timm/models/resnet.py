    ...
    def forward_features(self, x, dwt_ratio=None, post_norm=False, dwt_quant=1, dwt_drop=False):
        ...
        if self.mvar == False:
            ...
            for idx, item in enumerate(x):
                idx = 0
                item = self.bn1_lst[idx](item)
                item = self.act1(item)
                item = self.maxpool(item)

                def putin(layer, item, idx):
                    if isinstance(layer, MySequential):
                        for model_i in layer:
                            if isinstance(model_i, BasicBlock):
                                item = model_i(item, seq=idx)
                            else:
                                item = model_i(item)
                    return item

                if self.grad_checkpointing and not torch.jit.is_scripting():
                    item = checkpoint_seq([self.layer1, self.layer2, self.layer3, self.layer4], item, flatten=True)
                else:
                    item = putin(self.layer1, item, idx)
                    item = putin(self.layer2, item, idx)
                    item = putin(self.layer3, item, idx)
                    item = putin(self.layer4, item, idx)
                    
                result_lst.append(item)
            if self.dwt_level[0] == 2:
                return [tdwt.get_dwt_level2_inverse(result_lst), result_lst, gt_mvar]
            else:
                return [tdwt.get_dwt_level1_inverse(result_lst), result_lst, gt_mvar]
